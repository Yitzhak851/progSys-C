...
int main (int argc, char** argv) {
  ...
  /*** duplicate first arg into dynamically allocated string **/
  concat_args = strdup(argv[2]); /** attach args 3..argc-1 one by one **/
  // argv[1] is first argument
  for(i=3; i<argc; i++) {
    concat_args = strattach(concat_args, argv[1]); /** attach separator     **/
    concat_args = strattach(concat_args, argv[i]); /** attach i'th argument **/
  }
  printf("%s\n", concat_args);
  free(concat_args);
  return 0;
